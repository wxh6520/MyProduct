...
class CanvasMainViewController: BaseViewController {

    var cgView: StrokeCGView!
    @IBOutlet weak var leftRingControl: RingControl!
    @IBOutlet weak var leftRingControlHeight: NSLayoutConstraint!
    @IBOutlet weak var leftRingControlWidth: NSLayoutConstraint!
    @IBOutlet weak var leftRingControlLeading: NSLayoutConstraint!
    @IBOutlet weak var leftRingControlTop: NSLayoutConstraint!
    ...
    override func viewDidLoad() {
        ...
        title = "签名"
        let clearBtn = UIBarButtonItem(title: "clear", style: .done, target: self, action: #selector(clearButtonAction(_:)))
        pencilButton = UIButton(frame: CGRect(x: 0, y: 0, width: 75, height: 28))
        pencilButton.setTitle("pencil", for: .normal)
        pencilButton.setImage(UIImage(named: "Close"), for: .normal)
        pencilButton.addTarget(self, action: #selector(stopPencilButtonAction(_:)), for: .touchUpInside)
        let pencilBarBtn = UIBarButtonItem(customView: pencilButton)
        
        self.navigationItem.rightBarButtonItems = [clearBtn, pencilBarBtn]
